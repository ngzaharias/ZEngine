...
public:
	...
	constexpr bool operator==(const Rotator& rhs) const noexcept { return (m_Pitch == rhs.m_Pitch) && (m_Yaw == rhs.m_Yaw) && (m_Roll == rhs.m_Roll); }
	constexpr bool operator!=(const Rotator& rhs) const noexcept { return (m_Pitch != rhs.m_Pitch) || (m_Yaw != rhs.m_Yaw) || (m_Roll != rhs.m_Roll); }

	constexpr Rotator operator+(const Rotator& rhs) const noexcept { return Rotator(m_Pitch + rhs.m_Pitch, m_Yaw + rhs.m_Yaw, m_Roll + rhs.m_Roll); }
	constexpr Rotator operator-(const Rotator& rhs) const noexcept { return Rotator(m_Pitch - rhs.m_Pitch, m_Yaw - rhs.m_Yaw, m_Roll - rhs.m_Roll); }

	constexpr Rotator& operator+=(const Rotator& rhs) noexcept { m_Pitch += rhs.m_Pitch; m_Yaw += rhs.m_Yaw; m_Roll += rhs.m_Roll; return *this; }
	constexpr Rotator& operator-=(const Rotator& rhs) noexcept { m_Pitch -= rhs.m_Pitch; m_Yaw -= rhs.m_Yaw; m_Roll -= rhs.m_Roll; return *this; }

	constexpr Rotator operator*(const float rhs) const noexcept { return Rotator(m_Pitch * rhs, m_Yaw * rhs, m_Roll * rhs); }
	constexpr Rotator operator/(const float rhs) const noexcept { return Rotator(m_Pitch / rhs, m_Yaw / rhs, m_Roll / rhs); }

	constexpr Rotator& operator*=(const float rhs) noexcept { m_Pitch *= rhs; m_Yaw *= rhs; m_Roll *= rhs; return *this; }
	constexpr Rotator& operator/=(const float rhs) noexcept { m_Pitch /= rhs; m_Yaw /= rhs; m_Roll /= rhs; return *this; }

	constexpr Rotator operator-() const noexcept { return Rotator(-m_Pitch, -m_Yaw, -m_Roll); }
