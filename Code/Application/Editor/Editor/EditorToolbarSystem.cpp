...
namespace
{
	...
	void Draw_DebugGizmos(World& world)
	{
		...
		{
			auto& write = world.WriteComponent<eng::settings::DebugComponent>();
	...
	{
		const auto& settings = world.ReadComponent<editor::settings::LocalComponent>();
		const auto& axes = settings.m_Gizmos.m_CoordAxes;
		if (ButtonIcon("##axis", "Axes", icon::AXIS, ImVec2(22, 22), axes.m_IsEnabled))
		{
			auto& write = world.WriteComponent<editor::settings::LocalComponent>();
