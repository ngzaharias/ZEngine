...
void debug::settings::MenuSystem::Update(World& world, const GameTime& gameTime)
{
	...
	if (!hasWindow && world.HasAny<debug::settings::WindowEvent>())
	{
		...
		window.m_Engine = world.ReadComponent<eng::settings::DebugComponent>();
		window.m_Client = world.ReadComponent<client::settings::DebugComponent>();
		window.m_Hidden = world.ReadComponent<client::hidden::DebugComponent>();
		...
		if (ImGui::Begin("Debug Settings##gamedebug", &isWindowOpen, s_WindowFlags))
		{
			...
			if (inspector.Begin("##debugsettings"))
			{
				if (inspector.Write(window.m_Engine))
					world.WriteComponent<eng::settings::DebugComponent>() = window.m_Engine;
				ImGui::Spacing();
				if (inspector.Write(window.m_Client))
					world.WriteComponent<client::settings::DebugComponent>() = window.m_Client;
				ImGui::Spacing();
				if (inspector.Write(window.m_Hidden))
					world.WriteComponent<client::hidden::DebugComponent>() = window.m_Hidden;
				inspector.End();
			}
			ImGui::End();
		}
