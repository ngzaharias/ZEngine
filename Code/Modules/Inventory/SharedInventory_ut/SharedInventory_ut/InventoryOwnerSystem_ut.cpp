...
TEST_CASE("shared::inventory::OwnerSystem::Owner Added")
{
	RAIIHelper world;
	...
	SECTION("Owner Entity is Valid")
	{
		...
		REQUIRE(world.HasComponent<shared::inventory::OwnerComponent>(ownerEntity));
		const auto& ownerComponent = world.ReadComponent<shared::inventory::OwnerComponent>(ownerEntity);
	...
	SECTION("Owner Entity has multiple Storages, Same Frame")
	{
		...
		auto& requestAComponent = world.AddComponent<shared::inventory::StorageCreateRequestComponent>(requestAEntity);
		requestAComponent.m_Owner = ownerEntity;
		requestAComponent.m_Type = 0;
		auto& requestBComponent = world.AddComponent<shared::inventory::StorageCreateRequestComponent>(requestBEntity);
		...
		const auto& storageAComponent = world.ReadComponent<shared::inventory::StorageComponent>(resultAComponent.m_Storage);
		const auto& storageBComponent = world.ReadComponent<shared::inventory::StorageComponent>(resultBComponent.m_Storage);
	...
	SECTION("Owner Entity has multiple Storages, Next Frame")
	{
		...
		const auto& storageAComponent = world.ReadComponent<shared::inventory::StorageComponent>(storageAEntity);
		CHECK(storageAComponent.m_Owner == ownerEntity);

		const auto& storageBComponent = world.ReadComponent<shared::inventory::StorageComponent>(storageBEntity);
		CHECK(storageBComponent.m_Owner == ownerEntity);
	}
}

TEST_CASE("shared::inventory::OwnerSystem::Owner Removed")
{
	RAIIHelper world;
	...
	SECTION("Remove 1 of 1 Storage")
	{
		...
		CHECK(!world.HasComponent<shared::inventory::OwnerComponent>(ownerEntity));
	...
	SECTION("Destroy then Create, Same Frame")
	{
		...
		const auto& resultComponent = world.ReadComponent<shared::inventory::StorageCreateResultComponent>(requestBEntity);
		REQUIRE(resultComponent.m_Error == shared::inventory::EError::None);
