...
void shared::camera::Pan3DSystem::Update(World& world, const GameTime& gameTime)
{
	PROFILE_FUNCTION();

	const bool hasComponent = world.HasAny<ecs::query::Include<shared::camera::Pan3DComponent>>();
	...
	using CameraQuery = ecs::query
		...
		const shared::camera::Pan3DComponent>;
	...
	using AddedQuery = ecs::query
		::Added<const shared::camera::Pan3DTemplate>
		::Include<const shared::camera::Pan3DTemplate>;
	for (auto&& view : world.Query<AddedQuery>())
	{
		world.AddComponent<shared::camera::Pan3DComponent>(view);
	}

	for (auto&& view : world.Query<ecs::query::Removed<const shared::camera::Pan3DTemplate>>())
	{
		world.RemoveComponent<shared::camera::Pan3DComponent>(view);
	}
}