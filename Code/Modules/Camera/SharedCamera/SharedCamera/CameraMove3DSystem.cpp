...
void shared::camera::Move3DSystem::Update(World& world, const GameTime& gameTime)
{
	PROFILE_FUNCTION();

	if (world.HasAny<ecs::query::Added<shared::camera::Move3DComponent>>())
	{
		input::Layer layer;
	...
	using RemovedQuery = ecs::query
		...
		::Removed<shared::camera::Move3DComponent>;
	...
	using CameraQuery = ecs::query
		...
		const shared::camera::Move3DComponent>;

	const auto& settings = world.ReadComponent<shared::camera::SettingsComponent>();
	...
	using AddedQuery = ecs::query
		::Added<const shared::camera::Move3DTemplate>
		::Include<const shared::camera::Move3DTemplate>;
	for (auto&& view : world.Query<AddedQuery>())
	{
		world.AddComponent<shared::camera::Move3DComponent>(view);
	}

	for (auto&& view : world.Query<ecs::query::Removed<shared::camera::Move3DTemplate>>())
	{
		world.RemoveComponent<shared::camera::Move3DComponent>(view);
	}
}