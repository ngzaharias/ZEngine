...
void shared::camera::Move2DSystem::Update(World& world, const GameTime& gameTime)
{
	PROFILE_FUNCTION();

	if (world.HasAny<ecs::query::Added<shared::camera::Move2DComponent>>())
	{
		input::Layer layer;
	...
	using RemovedQuery = ecs::query
		...
		::Removed<shared::camera::Move2DComponent>;
	...
	using CameraQuery = ecs::query
		...
		const shared::camera::Move2DComponent>;
	...
	using AddedQuery = ecs::query
		::Added<const shared::camera::Move2DTemplate>
		::Include<const shared::camera::Move2DTemplate>;
	for (auto&& view : world.Query<AddedQuery>())
	{
		world.AddComponent<shared::camera::Move2DComponent>(view);
	}

	for (auto&& view : world.Query<ecs::query::Removed<shared::camera::Move2DTemplate>>())
	{
		world.RemoveComponent<shared::camera::Move2DComponent>(view);
	}
}