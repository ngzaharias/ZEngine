...
namespace
{
	...
	AABB2f GetCameraZone(const eng::CameraComponent& camera, const eng::TransformComponent& transform, const Vector2u& windowSize)
	{
		...
		const Vector3f worldPosA = eng::ScreenToWorld(camera.m_Projection, cameraView, windowSize, pixelPosA);
		const Vector3f worldPosB = eng::ScreenToWorld(camera.m_Projection, cameraView, windowSize, pixelPosB);
...
void hexmap::RootSystem::Update(World& world, const GameTime& gameTime)
{
	...
	const bool cameraAdded = world.HasAny<ecs::query::Added<eng::CameraComponent>::Include<eng::TransformComponent>>();
	const bool cameraChanged = world.HasAny<ecs::query::Updated<eng::CameraComponent>::Include<eng::TransformComponent>>();
	const bool transformAdded = world.HasAny<ecs::query::Added<eng::TransformComponent>::Include<eng::CameraComponent>>();
	const bool transformChanged = world.HasAny<ecs::query::Updated<eng::TransformComponent>::Include<eng::CameraComponent>>();
