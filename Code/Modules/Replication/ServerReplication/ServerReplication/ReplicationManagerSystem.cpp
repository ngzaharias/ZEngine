...
void server::replication::ManagerSystem::Update(World& world, const GameTime& gameTime)
{
	...
	entitiesToUpdate.Add(world.Query<ecs::query::Added<shared::tilemap::AgentComponent>>());
	entitiesToUpdate.Add(world.Query<ecs::query::Removed<shared::tilemap::AgentComponent>>());
	...
	for (const ecs::Entity& entity : entitiesToUpdate)
	{
		...
		const bool wantComponent = 
			...
			world.HasComponent<shared::tilemap::AgentComponent>(entity) ||
