...
CATCH_REGISTER_ENUM(server::inventory::EError, 
	server::inventory::EError::None,
	server::inventory::EError::MemberDead,
	server::inventory::EError::MemberDuplicate,
	server::inventory::EError::MemberMissing,
	server::inventory::EError::MemberUnassigned,
	server::inventory::EError::OwnerDead,
	server::inventory::EError::OwnerDestroyed,
	server::inventory::EError::OwnerInvalid,
	server::inventory::EError::OwnerUnassigned,
	server::inventory::EError::StorageDead,
	server::inventory::EError::StorageDuplicate,
	server::inventory::EError::StorageMissing,
	server::inventory::EError::StorageUnassigned);

TEST_CASE("server::inventory::StorageSystem::Member Add")
{
	RAIIHelper world;
	...
	SECTION("Member is Unassigned Entity")
	{
		...
		// check result
		REQUIRE(world.HasComponent<server::inventory::MemberAddResultComponent>(requestAEntity));
		const auto& resultComponent = world.ReadComponent<server::inventory::MemberAddResultComponent>(requestAEntity);
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Member == ecs::Entity::Unassigned);
		CHECK(resultComponent.m_Error == server::inventory::EError::MemberUnassigned);
	}
	...
	SECTION("Member is Dead Entity")
	{
		...
		CHECK(resultComponent.m_Error == server::inventory::EError::MemberDead);
	}

	SECTION("Member already in Storage")
	{
		// make a member
		world.AddComponent<server::inventory::MemberComponent>(memberEntity);
		world.Update();
		...
		CHECK(resultComponent.m_Error == server::inventory::EError::MemberDuplicate);
	}
	...
	SECTION("Storage is Unassigned Entity")
	{
		...
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Member == memberEntity);
		CHECK(resultComponent.m_Error == server::inventory::EError::StorageUnassigned);
	}
	...
	SECTION("Storage is Dead Entity")
	{
		...
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Member == memberEntity);
		CHECK(resultComponent.m_Error == server::inventory::EError::StorageDead);
	}
	...
	SECTION("Storage Destroyed in Previous Frame")
	{
		...
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Member == memberEntity);
		CHECK(resultComponent.m_Error == server::inventory::EError::StorageMissing);
	}
	...
	SECTION("Duplicate Request in Previous Frame")
	{
		...
		// check result a
		REQUIRE(world.HasComponent<server::inventory::MemberAddResultComponent>(requestAEntity));
		const auto& resultAComponent = world.ReadComponent<server::inventory::MemberAddResultComponent>(requestAEntity);
		CHECK(resultAComponent.m_TransactionId == transactionId);
		CHECK(resultAComponent.m_Member == memberEntity);
		CHECK(resultAComponent.m_Error == server::inventory::EError::None);

		world.Update();

		// check result b
		REQUIRE(world.HasComponent<server::inventory::MemberAddResultComponent>(requestBEntity));
		const auto& resultBComponent = world.ReadComponent<server::inventory::MemberAddResultComponent>(requestBEntity);
		CHECK(resultBComponent.m_TransactionId == transactionId);
		CHECK(resultBComponent.m_Member == memberEntity);
		CHECK(resultBComponent.m_Error == server::inventory::EError::MemberDuplicate);
	}
	...
	SECTION("Regular Flow")
	{
		...
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Member == memberEntity);
		CHECK(resultComponent.m_Error == server::inventory::EError::None);

		// check member
		REQUIRE(world.HasComponent<server::inventory::MemberComponent>(resultComponent.m_Member));
		const auto& memberComponent = world.ReadComponent<server::inventory::MemberComponent>(resultComponent.m_Member);
		...
		// checks changes
		const auto& changesComponent = world.m_EntityWorld.ReadComponent<server::inventory::StorageChangesComponent>();
...
TEST_CASE("server::inventory::StorageSystem::Member Remove")
{
	RAIIHelper world;
	...
	SECTION("Member is Unassigned Entity")
	{
		...
		// check result
		REQUIRE(world.HasComponent<server::inventory::MemberRemoveResultComponent>(requestAEntity));
		const auto& resultComponent = world.ReadComponent<server::inventory::MemberRemoveResultComponent>(requestAEntity);
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Member == ecs::Entity::Unassigned);
		CHECK(resultComponent.m_Error == server::inventory::EError::MemberUnassigned);
	...
	SECTION("Storage was Destroyed in Previous Frame")
	{
		...
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Member == memberEntity);
		CHECK(resultComponent.m_Error == server::inventory::EError::MemberMissing);

		// check storage
		CHECK(!world.HasComponent<server::inventory::StorageComponent>(storageEntity));
	}
	...
	SECTION("Storage was Destroyed in Same Frame")
	{
		...
		// check result
		REQUIRE(world.HasComponent<server::inventory::MemberRemoveResultComponent>(requestBEntity));
		const auto& resultComponent = world.ReadComponent<server::inventory::MemberRemoveResultComponent>(requestBEntity);
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Member == memberEntity);
		CHECK(resultComponent.m_Error == server::inventory::EError::None);
	...
	SECTION("Duplicate Request in Previous Frame")
	{
		...
		// check result a
		REQUIRE(world.HasComponent<server::inventory::MemberRemoveResultComponent>(requestAEntity));
		const auto& resultAComponent = world.ReadComponent<server::inventory::MemberRemoveResultComponent>(requestAEntity);
		CHECK(resultAComponent.m_TransactionId == transactionId);
		CHECK(resultAComponent.m_Member == memberEntity);
		CHECK(resultAComponent.m_Error == server::inventory::EError::None);
		...
		// check result b
		REQUIRE(world.HasComponent<server::inventory::MemberRemoveResultComponent>(requestBEntity));
		const auto& resultBComponent = world.ReadComponent<server::inventory::MemberRemoveResultComponent>(requestBEntity);
		CHECK(resultBComponent.m_TransactionId == transactionId);
		CHECK(resultBComponent.m_Member == memberEntity);
		CHECK(resultBComponent.m_Error == server::inventory::EError::MemberMissing);
	...
	SECTION("Duplicate Request in Same Frame")
	{
		...
		CHECK(resultBComponent.m_TransactionId == transactionId);
		CHECK(resultBComponent.m_Member == memberEntity);
		CHECK(resultBComponent.m_Error == server::inventory::EError::MemberDead);
...
TEST_CASE("server::inventory::StorageSystem::Storage Create")
{
	RAIIHelper world;
	...
	SECTION("Failure: Duplicate Storage, Same Frame")
	{
		// request create
		{
			auto& requestComponent = world.AddComponent<server::inventory::StorageCreateRequestComponent>(requestAEntity);
			requestComponent.m_TransactionId = transactionId;
			requestComponent.m_Owner = ownerEntity;
		...
		// check result a
		REQUIRE(world.HasComponent<server::inventory::StorageCreateResultComponent>(requestAEntity));
		...
		CHECK(resultBComponent.m_Error == server::inventory::EError::StorageDuplicate);
	}
	...
	SECTION("Success")
	{
		...
		// check result
		REQUIRE(world.HasComponent<server::inventory::StorageCreateResultComponent>(requestAEntity));
		const auto& resultComponent = world.ReadComponent<server::inventory::StorageCreateResultComponent>(requestAEntity);
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Storage != ecs::Entity::Unassigned);
		CHECK(resultComponent.m_Error == server::inventory::EError::None);

		// check storage
		REQUIRE(world.HasComponent<server::inventory::StorageComponent>(resultComponent.m_Storage));
		const auto& storageComponent = world.ReadComponent<server::inventory::StorageComponent>(resultComponent.m_Storage);
...
TEST_CASE("server::inventory::StorageSystem::Storage Destroy")
{
	RAIIHelper world;
	...
	SECTION("Failure: Unassigned Storage Entity")
	{
		...
		// check result
		REQUIRE(world.HasComponent<server::inventory::StorageDestroyResultComponent>(requestAEntity));
		const auto& resultComponent = world.ReadComponent<server::inventory::StorageDestroyResultComponent>(requestAEntity);
		CHECK(resultComponent.m_TransactionId == transactionId);
		CHECK(resultComponent.m_Storage == ecs::Entity::Unassigned);
		CHECK(resultComponent.m_Error == server::inventory::EError::StorageUnassigned);
	}
	...
	SECTION("Failure: Duplicate Request, Same Frame")
	{
		...
		// request destroy
		{
			auto& requestComponent = world.AddComponent<server::inventory::StorageDestroyRequestComponent>(requestBEntity);
			requestComponent.m_TransactionId = transactionId;
			requestComponent.m_Storage = storageEntity;
		}
		...
		// check result a
		REQUIRE(world.HasComponent<server::inventory::StorageDestroyResultComponent>(requestAEntity));
		const auto& resultAComponent = world.ReadComponent<server::inventory::StorageDestroyResultComponent>(requestAEntity);
		CHECK(resultAComponent.m_TransactionId == transactionId);
		CHECK(resultAComponent.m_Storage == storageEntity);
		CHECK(resultAComponent.m_Error == server::inventory::EError::None);

		// check result b
		REQUIRE(world.HasComponent<server::inventory::StorageDestroyResultComponent>(requestBEntity));
		const auto& resultBComponent = world.ReadComponent<server::inventory::StorageDestroyResultComponent>(requestBEntity);
		CHECK(resultBComponent.m_TransactionId == transactionId);
		CHECK(resultBComponent.m_Storage == storageEntity);
		CHECK(resultBComponent.m_Error == server::inventory::EError::StorageDead);
