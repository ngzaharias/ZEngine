...
namespace
{
	...
	using World = editor::trajectory::WindowSystem::World;
		...
		if (ImGui::BeginMenuBar())
		{
			if (ImGui::BeginMenu("File"))
			{
				if (ImGui::MenuItem("New"))
				{
					...
					auto& windowComponent = world.WriteComponent<editor::trajectory::WindowComponent>(entity);
					windowComponent.m_Asset = trajectory;
				}

				if (ImGui::MenuItem("Open"))
					world.AddComponent<editor::trajectory::AssetOpenComponent>(entity);

				if (ImGui::MenuItem("Save"))
					world.AddComponent<editor::trajectory::AssetSaveComponent>(entity);
	...
	void DrawPopupOpen(World& world, const ecs::Entity& entity)
	{
		...
		if (world.HasComponent<editor::trajectory::AssetOpenComponent>(entity))
		{
			world.RemoveComponent<editor::trajectory::AssetOpenComponent>(entity);
		}
	};
	...
	void DrawPopupSave(World& world, const ecs::Entity& entity)
	{
		...
		if (world.HasComponent<editor::trajectory::AssetSaveComponent>(entity))
		{
			auto& windowComponent = world.WriteComponent<editor::trajectory::WindowComponent>(entity);
			...
			world.RemoveComponent<editor::trajectory::AssetSaveComponent>(entity);
		}
	};
}
...
void editor::trajectory::WindowSystem::Update(World& world, const GameTime& gameTime)
{
	...
	for (const auto& request : world.Events<const editor::trajectory::WindowEvent>())
	{
		const int32 identifier = m_WindowIds.Borrow();
		const ecs::Entity windowEntity = world.CreateEntity();
		world.AddComponent<ecs::NameComponent>(windowEntity, "Trajectory Editor");

		auto& window = world.AddComponent<editor::trajectory::WindowComponent>(windowEntity);
	...
	for (auto&& view : world.Query<ecs::query::Include<editor::trajectory::WindowComponent>>())
	{
		auto& windowComponent = view.WriteRequired<editor::trajectory::WindowComponent>();
