...
namespace
{
	using World = editor::table::WindowSystem::World;
	...
	void DrawBrowser(World& world, const ecs::Entity& entity)
	{
		if (ImGui::CollapsingHeader("Tables: "))
		{
			...
			auto& windowComponent = world.WriteComponent<editor::table::WindowComponent>(entity);
	...
	void DrawInspector(World& world, const ecs::Entity& entity)
	{
		...
		if (ImGui::BeginTabBar("##tables"))
		{
			for (auto&& [index, tableName] : enumerate::Forward(windowComponent.m_TablesOpened))
			{
				...
				{
					const editor::table::Schema& schema = windowComponent.m_TablesMap.Get(tableName);
					const auto flags =
					...
					{
						for (const editor::table::Field& field : schema)
...
void editor::table::WindowSystem::Update(World& world, const GameTime& gameTime)
{
	...
	for (const auto& request : world.Events<const editor::table::WindowEvent>())
	{
		const int32 identifier = m_WindowIds.Borrow();
		const ecs::Entity windowEntity = world.CreateEntity();
		world.AddComponent<ecs::NameComponent>(windowEntity, "Table Editor");

		auto& window = world.AddComponent<editor::table::WindowComponent>(windowEntity);
		...
		// Simple
		{
			...
			editor::table::Schema& schema = window.m_TablesMap[tableName];
	...
	for (auto&& view : world.Query<ecs::query::Include<editor::table::WindowComponent>>())
	{
		auto& windowComponent = view.WriteRequired<editor::table::WindowComponent>();
