...
void editor::input::WindowSystem::Update(World& world, const GameTime& gameTime)
{
	...
	for (const auto& request : world.Events<const editor::input::WindowEvent>())
	{
		const int32 identifier = m_WindowIds.Borrow();
		const ecs::Entity windowEntity = world.CreateEntity();
		world.AddComponent<ecs::NameComponent>(windowEntity, "Input Editor");

		auto& window = world.AddComponent<editor::input::WindowComponent>(windowEntity);
	...
	for (auto&& view : world.Query<ecs::query::Include<editor::input::WindowComponent>>())
	{
		auto& window = view.WriteRequired<editor::input::WindowComponent>();
